...
async function readJsonBody(
  ...
  for await (const chunk of request.body) {
    if (request.signal.aborted) {
      throw new StreamRouteError(499, "cancelled", "The stream request was cancelled.");
    }
...
async function* validateEventStream(
  ...
  try {
    ...
    try {
      ...
      // Closing an injected channel must not keep the authenticated response open.
    }
    if (requestSignal.aborted && !operationController.signal.aborted) {
