> 非权威草稿：等待 Legacy Audit 与 Upstream Audit 最终报告后重新核对。不得用于 OpenSpec apply。
...
- Incorporate completed upstream and legacy audit evidence into provenance and compatibility records before the change is marked complete.
...
- `packages/mock-relay` (`@routego-image/mock-relay`): deterministic Web API request handling and an optional loopback test server. It records only sanitized request shapes.

Root commands will cover `build`, `typecheck`, `test`, `test:browser`, and repository safety checks. Packages build to ESM plus declarations with `tsup`; TypeScript project references and package exports prevent downstream source duplication. Runtime dependencies remain pure JavaScript.
...
- provider protocol tiers, capability names, four-state values, evidence, timestamps, and redacted endpoint descriptors;
- structured service errors and result envelopes, including partial success and `degradedContinuation`;
...
Provider protocol is explicit:

- `single-endpoint-json` (Tier A): the configured endpoint; text generation is the only default route. Image data URL support is a separate capability.
...
The Foundation mock service implements the same interface with deterministic results. The mock relay separately emulates upstream provider shapes. This distinction lets Studio mock the application service while Creation tests provider adapters without either lane owning the other's code.
...
- recursive diagnostics redaction normalizes key names and removes API keys, authorization/cookie headers, bearer values, session tokens, credential-bearing query parameters, and image data payloads;
...
All boundaries use a structured error with a stable code, stage, safe user message, retryable flag, optional capability, and sanitized details. Parsing failures are `invalid_request`; unavailable provider paths are `capability_unavailable`; session/origin failures are authorization errors; provider-specific failures remain distinguishable without exposing request headers or raw bodies.
...
Vitest covers Zod contracts, cross-field validation, capability state transitions, routing, redaction, loopback/origin/session policy, path containment, service dispatch, and every mock relay mode. Playwright's request client performs a browser-stack smoke test against the loopback mock server. CI runs install, safety, typecheck, build, unit/contract tests, and browser smoke tests on Windows, Ubuntu, and macOS with Node 20.19+.

The upstream audit must provide a concrete `CookSleep/gpt_image_playground` commit and MIT attribution before Foundation records it in `THIRD_PARTY_NOTICES.md` and the provenance document. The legacy audit must provide evidence-backed compatibility behavior before Foundation freezes the compatibility checklist. Missing audit evidence blocks only those tasks and final completion; placeholders are forbidden.
...
- Foundation contract/security/browser test directories;
...
- [Exact upstream/legacy facts arrive after planning] → Treat audit documents as required evidence inputs and block provenance/compatibility tasks until their commits are available.
...
2. Integrate completed audit documentation/commits without editing their evidence.
...
- What exact upstream commit and attribution text will the Upstream Reuse Audit approve?
- Which evidence-backed legacy edge cases will the Legacy Plugin Audit require in the final compatibility checklist?

These questions do not change the architecture, but their audit outputs are mandatory before the related tasks and the change can be completed.
