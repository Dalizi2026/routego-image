...
Schema-validated public success results use an operation-aware public-result projection rather than the diagnostic redactor. Non-image operations, including `routego_open_studio`, preserve every frozen-schema field in structured MCP text; the Studio result therefore remains `routegoOpenStudioResultSchema`-valid and immediately consumable with its fresh one-time token. Generate, edit, and batch use an explicitly image-payload-free metadata projection: it preserves every non-payload public field, artifact identity/slot/phase/MIME/metadata, relationship, failure fact, and real existing public path, but omits `display.dataUrl`, binary payloads, and any display object left empty by that removal. A valid pathless dataUrl-only artifact remains pathless in text and is not required to re-satisfy the original image-result schema after its only payload field is intentionally removed; the projector never fabricates a path or token. Validated final bytes remain available only as MCP image content.

Structured errors, caught exceptions, framing failures, logger output, Authorization values, credentials, arbitrary diagnostic URLs, and binary diagnostic data continue through recursive diagnostic redaction. The operation-aware success projector never reclassifies those diagnostic/error boundaries as public success.
