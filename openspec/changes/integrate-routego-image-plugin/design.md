...
The workbench shows protected partial artifacts as they arrive. `completed` promotes the validated result. `failed` after partial output retains those artifacts and matching `receivedAnyOutput=true` and `mayHaveBilled=true`; it never offers automatic safe replay. Invalid framing/schema/sequence fails closed without rendering unvalidated data. Reader/channel cleanup is separate from resource lifetime: every validated ephemeral partial descriptor expires five minutes after registration, and terminal failure, disconnect, cancel, or unmount does not shorten that TTL. A client may explicitly release a resource only after removing every UI reference. Process shutdown revokes all ephemeral resources immediately; expiry or shutdown then makes the protected fetch fail safely.
...
Creation output data URLs are decoded into an Integration-owned per-request transaction directory, revalidated for MIME, size, dimensions, and SHA, and never logged. When `saveToLibrary=true`, Integration ingests source, partial, and final renditions plus exact relationships in one Library transaction, then resolves/registers protected resources. Existing Library inputs remain relationships to their original assets. When Studio does not save, final/partial resources remain in a bounded Integration ephemeral registry for the descriptor's fixed five-minute TTL and are not added to the Library. Stream termination closes the event channel but does not revoke a referenced partial before expiry. A public operation with `saveToLibrary=false` must provide an approved output directory; otherwise it fails before provider submission so an unindexed durable file is not silently invented.
...
- [Risk] A streamed partial resource intentionally outlives a failed or disconnected stream. → Mitigation: a fixed five-minute descriptor TTL, session/origin-protected fetch, explicit safe client release, independent reader/channel cleanup, shutdown revocation, and expiry/disconnect/cancel regression tests.
