...
- Add the PD-005 prerequisite Studio correction before composition: an authenticated Integration fetch-stream route plus a browser-safe Studio SSE parser/gateway/workbench flow with strict content type, UTF-8 framing, schema and monotonic-sequence validation, bounded frames, terminal enforcement, abort cleanup, deterministic mock streaming, and honest failure-after-partial billing/output presentation.
- Complete Integration-owned image-output handling, including non-overwriting project copies and explicitly reported simple chromakey transparency post-processing, without claiming native transparency or complex-background support when the configured relay has not proven it.
...
- No addition or renaming of public MCP tools and no main-spec, Library, or Studio change beyond the five PD-005 modified capabilities and their exact prerequisite implementation/test seams. Creation provider/executor internals remain read-only.
...
- `studio-creation-workbench`: Consume authenticated validated fetch-stream events, render arriving partials, promote terminal success, preserve failure-after-partial billing/output risk, and clean up on cancellation or invalid streams.

## Impact

- Expected product scope: the exact PD-005 contract/Library/Studio prerequisite files and focused tests, a new Integration-owned runtime/package area, plugin manifest and Skill files, packaging/install/release scripts, integration tests, CI workflow, and only the root dependency/workspace/lockfile changes justified by the approved design.
- Authorized cross-lane impact is limited to the PD-005 Library rendition/ingestion/read/ZIP seams, Studio Library retry seam, Studio API/creation streaming state, deterministic mock bridge, and focused/browser tests enumerated in `tasks.md`. All other Creation, Library, Studio, shared, Foundation, and main-spec files remain read-only without a new `[PLAN_DEVIATION]`.
