...
- Separate stream/channel cleanup from protected partial-resource lifetime: terminal, disconnect, cancel, or unmount closes readers and event channels, while already validated partial resources remain fetchable through their five-minute descriptor TTL unless the client explicitly releases an unreferenced resource; process shutdown revokes all ephemeral resources.
...
- No addition or renaming of public MCP tools, rendition/relationship role invented for chromakey, or main-spec, Library, or Studio change beyond the five PD-005 modified capabilities and their exact prerequisite implementation/test seams. Creation provider/executor internals remain read-only.
...
- `studio-creation-workbench`: Consume the exact authenticated stream route with a first/unique `started` state machine, render arriving partials, promote terminal success, preserve failure-after-partial billing/output risk and five-minute protected-resource lifetime, and clean up channels without premature resource revocation.
