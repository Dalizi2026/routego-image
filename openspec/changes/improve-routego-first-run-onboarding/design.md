...
The first-run screen is a connection task, not a capability-debug dashboard. Its primary hierarchy will explain that Studio is used for local configuration while ordinary generation happens directly in Codex, then show only a call-endpoint field, a write-only API-key field, and one explicit connect-and-fetch-models action. The client will recognize a complete generations route versus an API base, preserve the exact entered call target, and derive the corresponding models route internally for the user-requested refresh. Existing profiles without a key enter replacement mode automatically; profiles with a key keep it when the visible field is left empty. A successful refresh reveals a compact model selector and finish action. A failed refresh exposes only a manual-model fallback and a safe error, not the technical endpoint/profile form. Calibration, evidence, defaults, output, and multi-profile controls do not appear in first run. No first-run render performs provider traffic.
