...
5. For an ordinary text-only generation (`kind: "generate"` with no reference, target, mask, or other image input), unknown `text-generation` evidence is an allowed baseline: after `configured: true`, call `routego_generate` once and let the provider response establish the actual result. Do not request a capability probe for this baseline. Do not add optional quality, size, aspect-ratio, format, variant-count, transparency, moderation, or partial-image controls unless the user explicitly asks for them; use the schema defaults (`auto`, PNG, one output) for an ordinary request. If the user explicitly asks for an optional control whose capability is unknown, explain that it requires explicit capability approval or ask whether to remove that control; never silently add it. If `text-generation` is explicitly unsupported, stop and report that verified limitation. For edits, references, masks, and other capability-dependent operations, unknown, unsupported, or degraded evidence remains a verified limitation and the dependent operation must stop. Do not imply support or fabricate success. 对于普通文字生图（`kind: "generate"` 且没有参考图、目标图、遮罩或其他图像输入），`text-generation` 能力未知属于允许的基础状态：只要 `configured: true`，就调用一次 `routego_generate`，由服务商实际响应确定结果；不要为这条基础路径要求能力探测。除非用户明确提出，否则不要自动加入质量、尺寸、宽高比、格式、变体数、透明、审核或部分图像等可选控制；普通请求使用模式默认值（`auto`、PNG、单张）。如果用户明确要求的可选控制尚未验证，说明它需要明确的能力授权，或询问是否去掉该控制；绝不能静默添加。如果明确返回不支持，才停止并报告已验证的限制。编辑、参考图、遮罩及其他依赖能力的操作仍然必须有对应证据；能力未知、不支持或降级时停止，不得假装支持或伪造成功。
