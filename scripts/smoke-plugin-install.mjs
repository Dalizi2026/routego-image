...
async function exerciseSyntheticInstalledRuntime(paths, folderId) {
  ...
  try {
    ...
    if (initialized?.serverInfo?.name !== "routego-image" ||
        initialized?.serverInfo?.version !== "1.0.7") {
      fail("the isolated synthetic runtime did not initialize as Routego Image 1.0.7");
    }
...
async function runPortableInstalledProcess(options, paths, verification, copiedVerification) {
  ...
  try {
    ...
    if (initialized?.serverInfo?.name !== "routego-image" ||
        initialized?.serverInfo?.version !== "1.0.7") {
      fail("the portable installed process did not initialize Routego Image 1.0.7");
    }
    ...
    if (status.configured !== false || status.hasApiKey !== false ||
        status.service?.status !== "ready" || status.service?.version !== "1.0.7") {
...
async function runInsideOwnedRoot(options, root, verification) {
  ...
  try {
    ...
    if (server?.serverInfo?.name !== "routego-image" || server?.serverInfo?.version !== "1.0.7") {
      fail("fresh Codex did not discover Routego Image 1.0.7");
    }
